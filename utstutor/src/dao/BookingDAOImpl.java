...
public class BookingDAOImpl extends BaseDAO_Impl<Bookings, Booking> implements BookingDAO{
    ...
    public Bookings getBookings(Integer id, String subject, String studentEmail, String status ) {
        ...
        if(!isEmpty(subject)) results = results.findBySubject(subject);
        if(!isEmpty(studentEmail)) results = results.findByStudentEmail(studentEmail);
        if(!isEmpty(status)) results = results.findByStatus(status);
        return results;
    }
}
